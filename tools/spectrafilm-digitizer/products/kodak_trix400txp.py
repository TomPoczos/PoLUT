...
def _hd_chart():
    ...
    chart = ChartSpec(
        ...
        curves=[
            CurveSpec("3.5min", label_position_override=(361, 244)),
            CurveSpec("6.25min", label_position_override=(303, 218)),
            CurveSpec("9min", label_position_override=(341, 200)),
        ],
...
def _ci_chart():
    ...
    return ChartSpec(
        ...
        curves=[CurveSpec("HC-110", label_position_override=(420, 290))],
        film_id="_unused", region_bbox=region, extraction_method="vector_position",
...
def build_all():
    ...
    for dev_t in DEV_TIMES:
        ...
        dm.plot_fit_qa(xs_by_dev[dev_t], ys_by_dev[dev_t], fit, grids.LOG_EXPOSURE,
                        title=f"Kodak Tri-X Pan Professional (TXP), HC-110B {dev_t:g} min (net density, above base)",
