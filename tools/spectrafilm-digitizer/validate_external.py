"""
Runs a produced `profile.spektrafilm.json` (the real development-time
family, spektrafilm's own "source profile" shape) through the REAL,
installed `spektrafilm` package as an external validation oracle --
authoritative correctness from the actual dependency, not a reimplementation
of it (see project plan design decision #2).
...
IMPORTANT, confirmed by direct testing (not assumed): only
`profile.spektrafilm.json` is meaningfully checked here. The installed
`profiles_loader`'s `_validate_profile` has moved *ahead* of darktable's own
bundled C reader -- it now expects a BW stock's `log_sensitivity`/
`channel_density` to stay single-column (`n_channels == 1`, a property
derived from `channel_model`), never widened to 3 identical RGB columns.
darktable's `spektra_sim.c` (`json_read_dmatrix(..., SF_NWL, 3)`) is still on
the *older* always-3-columns convention `spektrafilm_export_data.py`
implements, and rejects (or rather, the Python validator rejects) our
correctly-widened `profile.darktable.json` for exactly that reason -- a real,
confirmed skew between the two consumers' current schema versions, not a bug
in our output. So `profile.darktable.json` is validated instead by
`spektra_profile.validate_darktable_pack()`, which mirrors the C reader's
own requirements directly (see that function) -- the authoritative check for
the file that actually matters to darktable.
"""
