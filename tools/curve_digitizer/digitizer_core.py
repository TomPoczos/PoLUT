...
def extract_traces_in_region(page, region_bbox, min_points=12, stroke_rgb=None, tol=0.15, continuity_tol=1.5,
    ...
    f9-Tri-X_Pan.pdf's TXT/HC-110 panel (spectrafilm-digitizer's
    kodak_trix400txt.py): the 7min and 8.5min curves each lose their last
    ~5 points -- right at the shoulder, next to the chart's own right
    border -- as a separate tiny drawing object, and the two curves' own
    tail fragments sit only ~3px apart from EACH OTHER in y, closer to
    each other than either is to its own true parent's endpoint (7.2px vs
    8.6px -- and backwards: the closer raw distance is the WRONG pairing).
    `merge_strategy="proximity"` either mismatches or (with
    `strict_chain_merge`) transitively fuses both curves' tails into one
    bogus merged trace, since raw distance can't discriminate two
    candidates that are both plausibly "close enough." What does
    discriminate them is each parent curve's own local trajectory: fit a
    line through the last `slope_fit_points` real points at each candidate
    main trace's (>= min_points) own open end, extrapolate it across the
    gap, and score each orphan fragment (`slope_min_frag_points` <= len <
    min_points -- big enough to not be tick-mark/leader-line noise, too
    small to already count as a real trace) by how well ITS points agree
