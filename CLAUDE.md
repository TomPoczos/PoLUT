...
4. **Cascade builders** — `build_trix_cascade()` composes the negative curve with a paper grade into a single exposure→reflectance transfer function (anchored so 18% grey lands correctly). `build_velvia_layer()` does the equivalent for one reversal dye layer, plus a parametric contrast (`VELVIA_GAMMA`) since no real multi-grade reversal print data exists.
...
- `GREY = 0.18` is the anchor point every transfer function is normalized around; both cascade builders search their curve data for the log-exposure that reproduces 18% output.
