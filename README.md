...
### Why the correction is scoped to the straight line, not applied to the whole curve

An earlier version of this fix applied one scalar to the *entire* digitized curve — toe, straight-line middle, and shoulder all rescaled by the same factor. That over-reached what Jones's own theory claims: his product rule is explicit about its scope, *"for the straight line portions where gradient is constant and replaceable by gamma"* (p.64) — he never claims it governs the toe or shoulder. A uniform scalar nonetheless flattens the toe/shoulder too, and since those regions are already lower-gamma than the straight line by definition, scaling them down *again* by the same factor flattens them far more than the theory justifies. Measured effect: on Kodachrome 64 × Radiance III, the uniformly-corrected curve's own maximum density fell to 2.72 against the real digitized curve's 3.68, and the resulting print's shadow density topped out at 1.83 against Radiance III's own real digitized Dmax of 2.52 — about a stop of the paper's own real black reserve going unused, which is exactly why shadows read as washed-out grey instead of black despite the highlight side looking correct.

Inserting a second real material (e.g. EASTMAN Fine Grain Duplicating Panchromatic Negative 2234/5234, `papers/kodak_finegrain_duplicating_pan_2234_TI0147.pdf`, a real Kodak masking/duplicating stock with published, development-time-adjustable gamma) as an intermediate cascade stage was considered and rejected: it is, like EASTMAN Color Internegative, itself a duplicating stock designed around some assumed low-contrast "master positive" input — inserting it would relocate the exact real-vs-duplicating-positive mismatch this whole correction exists to escape, not resolve it.

`gamma_correct_curve()` instead rescales density *only* inside the curve's own straight-line window (`_straight_line_window()` — the same window `_measured_gamma()` and `density_midpoint()` already read gamma and reference density from, so this is a third use of one existing definition of "straight line," not a new one), by whatever factor brings (this curve's own straight-line gamma) × (that specific paper's own straight-line gamma) to `GAMMA_CORRECT_TARGET`. Outside that window — the toe and shoulder — density is carried forward *additively* using the curve's own real, unmodified deltas from the manufacturer's own digitized data, continuing from wherever the corrected straight-line segment ends. Toe and shoulder therefore keep their true measured shape and reach the film's own real Dmin/Dmax exactly as digitized — no material borrowed, nothing invented past the boundary. There is one real, deliberate discontinuity — local gamma steps at the window boundary (corrected inside, the curve's own original gamma outside) — but every curve in this file is already represented and evaluated as piecewise-linear between digitized sample points, so one additional slope discontinuity at a principled boundary is consistent with how this file already represents every other curve, not a new kind of artifact.

**The result, measured the same way.** Same film/paper/target: the corrected curve's own maximum density recovers to 2.97 (versus 2.72 for the old uniform rescale, versus 3.68 for the real uncorrected film), and the print's own shadow density reaches 2.12 against Radiance III's real 2.52 Dmax — roughly two-thirds of a stop of the previously-wasted black reserve recovered, using only data already digitized in this file, plus the light-surround target from Bartleson & Breneman. Kodachrome 64's red layer through Radiance III, corrected: grey holds exactly at 0.18, -3 EV reads 0.0123, -4 EV floors at 0.0092 (paper density 2.12) — compare to 0.033 at -2 EV / floor 0.0181 (paper density 1.83) from the first, uniform-rescale version of this fix, and 0.005 at -1.5 EV uncorrected.

| Film | RadianceIII span | IlfochromeM span | IlfochromeP span |
|---|---|---|---|
| Velvia 50 | 0.657 | 0.681 | 0.691 |
| Kodachrome 64 | 0.712 | 0.725 | 0.734 |
| Fuji Provia 100F | 0.741 | 0.746 | 0.754 |
| Kodak Ektachrome 100D | 0.730 | 0.738 | 0.749 |

(Span = encoded white-corner minus black-corner output, same metric and methodology as "Choosing a print paper"'s table above; measured on the original target=1.0 uniform-rescale version of this fix, since superseded by target=1.1 scoped-to-straight-line above — kept here as the reference point for "these spans read lower than the internegative route's 0.83-0.92," which remains true of the current version too.) A lower span at the two most extreme LUT corners means the curve takes more stops to get there, i.e. it's less crushed, not more. Ilfochrome Micrographic M needs the largest correction of the three (its own measured gamma, ~1.9-2.1, is the highest of any paper in this file) and was the specific case CLAUDE.md already records as rejected once for compounding too hard *uncorrected* — it is not re-rejected here; correcting it is the more direct test of whether this fix actually works, and it renders comparably to the other two once corrected.

**What this doesn't touch.** The internegative route (`COLOR_FILMS`/`_reversal_stage_fn()`) is left exactly as it was — it has the identical underlying gamma-product problem, just via a different, currently-uncorrected pairing, and is a separate piece of work. `NEGATIVE_FILMS` (Portra 400 etc.) don't need this: a camera negative's own native gamma is already low (that's what makes it a negative rather than a reversal stock), so pairing it with a real print paper doesn't overshoot Jones's target the way a reversal original does.
