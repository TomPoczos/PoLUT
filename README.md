...
### Why the target is 1.25, not 1.0 (and not 1.1)
...
Roufs, Koselka & van Tongeren's own experimental rig makes the right figure directly measurable rather than just asserted: their test chain was a **slide (reversal-film) scanner feeding a monitor** — almost exactly this project's own output path, a digitized film image displayed on a screen — and they report "the gamma of the slide scanner - monitor chain was about 1.2 in all cases," with "the optimal value for the effective gamma... about 1.2-1.3... very near what Bartleson and Breneman found for TV in 1967." `GAMMA_CORRECT_TARGET = 1.25` is the midpoint of that directly-measured, directly-applicable range — corrected from the earlier 1.1 once real-world use showed the reflection-print figure under-delivering contrast for a screen-viewed LUT.
