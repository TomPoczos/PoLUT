...
**What this project does instead.** `gamma_correct_curve()` applies Jones's own math directly to the already-digitized curve data, rather than simulating either physical mechanism. For each dye layer, it rescales the reversal film's own curve around its existing reference-density anchor (`density_midpoint()` — the same point already used to calibrate the stage's exposure handoff, so the anchor itself doesn't move) by whatever factor brings (that layer's own measured gamma) × (that specific paper layer's own measured gamma) to 1.0. The correction is neutral and magnitude-only — the same spectral neutrality a real flash has — applied identically in reasoning to all three layers, letting each dye layer's own real curve shape carry the rest. This is not a fabricated curve and it doesn't claim to reproduce a specific historical darkroom technique; it's labeled exactly as honestly as `density_midpoint()` itself already is elsewhere in this file: derived from real data and real (100-year-old, still-standard) theory, not read off a manufacturer spec sheet, because no manufacturer spec sheet answers this specific question.

**The result, measured the same way.** Kodachrome 64's red layer through Radiance III, corrected: output reaches only ~0.02 by -3 EV and ~0.86 by +3 EV (fully floors around -5 EV, fully ceilings around +5-6 EV) — compare to ~0.005 at -1.5 EV / ~0.93 at +1.5 EV uncorrected. That's roughly the film's own real captured latitude, not a manufactured one.
